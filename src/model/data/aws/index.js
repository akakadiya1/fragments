...
async function deleteFragment(ownerId, id) {
  ...
  try {
    // Delete metadata from DynamoDB and actual data from S3 in parallel
    await Promise.all([ddbDocClient.send(command), deleteFragmentData(ownerId, id)]);
    logger.info(`Successfully deleted fragment ${id} for owner ${ownerId}`);
  } catch (err) {
...
// Function to delete fragment data from S3
